...
# -----------------------------------
def mnist_model(input_size, num_labels):
    hidden_units = 256
    dropout = 0.45
    # ---- Model description
    ...
    return model
# -----------------------------------
def mnist_training(model, x_train, y_train):
...
def MNIST_dense():
    ...
    build_and_run(name)
    return
def MNIST_test(model, x_test, y_test):
  batch_size = 128
...
if __name__ == "__main__":
    ...
    model = mnist_model(input_size, num_labels)
    # MNISTbinW(model)
    mnist_training(model, x_train, y_train)
    MNIST_test(model, x_test, y_test)
    # MNIST_dense()
